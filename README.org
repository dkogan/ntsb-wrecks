...
$ sqlite3 -header -csv data.sqlite \
    'select events.ev_id,ev_year,acft_make,acft_model,acft_series,acft_serial_no,latitude,longitude,wx_obs_fac_id,wx_obs_dist,wx_obs_dir,printf("https://app.ntsb.gov/pdfgenerator/ReportGeneratorFile.ashx?EventID=%s&AKey=1&RType=HTML&IType=CA",events.ev_id) as url from events,aircraft where events.ev_id==aircraft.ev_id;' |
...
Now we have all the information, and we run another script to generate a =.kml=
that mapping tools can ingest. I cut it down to a small region because web
browsers are glacially slow. To see all the incidents in the San Gabriel
Mountains:

#+BEGIN_EXAMPLE
$ ./make-map.pl 34.08 -118.52 34.54 -117.38 joint.vnl > wrecks.kml
#+END_EXAMPLE
